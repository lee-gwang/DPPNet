...
from utils.rcp_utils import DPPConv2d, DynConv2d
...
import os, time, sys
import argparse
from model import resnet, dwpresnet, dynresnet
...
parser.add_argument('--min_lr', '-mlr', type=float, default=1e-6, help='min learning rate')
...
def generate_model(model_arch):
    ...
    elif model_arch == 'dwpresnet34':
        return dwpresnet.dwpresnet34(dataset=args.dataset, pretrained=args.pretrained)
                ...
                with autocast():
                    ...
                    for name, param in net.named_parameters():
                        if 'threshold' in name:
                            loss += args.alpha * torch.sum(torch.exp(-param))
...
def test_accu(testloader, net, device, logger, macs, types='Test'):
    ...
    # FLOPs
    ###########################
    results_keep = (sparsity_list[0] + macs)/(macs + sparsity_list[1]) *100
    results_reduct = (sparsity_list[1] - sparsity_list[0])/(macs + sparsity_list[1]) *100 # [keep, all]
    # accuracy
    logger.info('keep ratio: %.1f %%' % results_keep)
    logger.info('reduction ratio: %.1f %%' % results_reduct)
