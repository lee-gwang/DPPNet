...
from utils.rcp_utils import DynConv2d, DynConv2d2
try:
...
def conv3x3(in_planes, out_planes, stride=1, relu=False):
    """3x3 convolution with padding"""
    return nn.Sequential(
        DynConv2d2(in_planes, out_planes, kernel_size=3, stride=stride,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = DynConv2d2(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = DynConv2d2(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = DynConv2d2(inplanes, planes, kernel_size=1, stride=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = DynConv2d2(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = DynConv2d2(planes, planes * self.expansion, kernel_size=1, stride=1, bias=False)
